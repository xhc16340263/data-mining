...
def main():
    ...
    optimizer = torch.optim.Adam(policies, lr=args.lr, betas=(0.9, 0.999), eps=1e-08, weight_decay=args.weight_decay)
    
    if args.evaluate:
        validate(val_loader, model, criterion, 0)
        return

    log_training = open(os.path.join(args.root_log, '%s_adam.csv' % args.store_name), 'w')
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    torch.save(state, '%s/%s_adam_checkpoint.pth.tar' % (args.root_model, args.store_name))
    if is_best:
        shutil.copyfile('%s/%s_adam_checkpoint.pth.tar' % (args.root_model, args.store_name),'%s/%s_adam_best.pth.tar' % (args.root_model, args.store_name))
