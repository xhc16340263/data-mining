...
def validate(val_loader, model, criterion, iter, log):
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        for j in range(32):
          target = target_var[j]
          class_num[target] = class_num[target] + 1
          _, predict = torch.max(output[j],0) 
          if(predict == target):
            correct_num[target] = correct_num[target] + 1
          
          
        for k in range(174):
          acc[k] = correct_num[k]/(class_num[k])*100.0
          acc[k] = round(acc[k],2)
        ...
        if(i == 359):
            ...
            top5_acc, top5_idx = torch.topk(acc_tensor, 5)
            with open('out.txt','w')as f:
                print(class_num,file = f)
                print(correct_num,file = f)
                print(acc,file = f)
                print(top5_idx)
                print(top5_acc)
